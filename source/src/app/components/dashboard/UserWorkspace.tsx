...
export function UserWorkspace({ activeSection = 'dashboard', onSectionChange }: UserWorkspaceProps) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
          ...
          {/* 4. Age & Bio-Marker Card — FIXED CLEAN LAYOUT */}
          <div
            onClick={() => onSectionChange && onSectionChange('my-skin-profile')}
            style={{ borderRadius: '18px', background: '#fff', border: '1px solid #edeef4', padding: '18px 20px', boxShadow: '0 4px 16px -10px rgba(23,20,51,0.18)', cursor: 'pointer', transition: 'border-color 0.2s' }}
            onMouseEnter={e => (e.currentTarget.style.borderColor = PUR)}
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              <span style={{ fontSize: '0.72rem', fontWeight: 800, color: '#7c8199', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Age & Bio-Marker</span>
              <span style={{ fontSize: '0.65rem', fontWeight: 800, color: '#059669', background: '#ecfdf5', padding: '2px 7px', borderRadius: '6px', whiteSpace: 'nowrap' }}>-{chronologicalAge - calculatedSkinAge > 0 ? chronologicalAge - calculatedSkinAge : 0} Yrs Skin Age</span>
            </div>
            {/* Row 1: Chronological Age */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
              <span style={{ fontSize: '0.7rem', color: '#94a3b8', fontWeight: 600, minWidth: '52px' }}>Logged Age</span>
              <span style={{ fontSize: '1.6rem', fontWeight: 900, color: '#0f172a', lineHeight: 1 }}>{chronologicalAge}</span>
            </div>
            {/* Row 2: Skin Age */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
              <span style={{ fontSize: '0.7rem', color: '#94a3b8', fontWeight: 600, minWidth: '52px' }}>Skin Age</span>
              <span style={{ fontSize: '1.6rem', fontWeight: 900, color: '#059669', lineHeight: 1 }}>{calculatedSkinAge}</span>
              <span style={{ fontSize: '0.7rem', color: '#059669', fontWeight: 700, background: '#f0fdf4', padding: '2px 6px', borderRadius: '4px' }}>↓ Younger</span>
            </div>
            <div style={{ fontSize: '0.72rem', color: '#94a3b8', borderTop: '1px solid #f1f5f9', paddingTop: '8px' }}>{profileGender || 'Female'} · {fitzpatrickType.split(' ')[0] || 'Type IV'}</div>
          </div>
  ...
  const renderMyProfilePage = () => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {/* Profile Header Card — EXACT Admin Layout */}
        <Card style={{ padding: '24px' }}>
          <CardHead title="My Profile" right={<span style={{ padding: '4px 10px', borderRadius: '999px', background: `${PUR}18`, color: PUR, fontSize: '0.74rem', fontWeight: 700 }}>Verified Member</span>} />

          <div style={{ display: 'flex', gap: '20px', alignItems: 'center', padding: '8px 0 20px', borderBottom: '1px solid #f1f2f7' }}>
            {/* Avatar with camera dropdown — EXACT Admin */}
            ...
            <div>
              <div style={{ fontSize: '1.3rem', fontWeight: 800, color: '#171433' }}>{userName}</div>
              <div style={{ fontSize: '0.84rem', color: PUR, fontWeight: 600, marginTop: '3px' }}>Skin Health Member</div>
              <div style={{ fontSize: '0.8rem', color: '#a3a7bd', marginTop: '2px' }}>{userEmail}</div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginTop: '16px' }}>
            {[
              { label: 'Skin Health Score', value: `${scorePct || 82}/100`, color: PUR },
              { label: 'Skin Age', value: `${calculatedSkinAge} Years`, color: '#059669' },
              { label: 'Active Concerns', value: String(selectedConcerns.length), color: BLU },
              { label: 'Skin Type', value: `${currentSkinType}`, color: ORA },
            ].map((s, i) => (
        ...
        {/* Editable rows — EXACT Admin AccountSettings style */}
        <Card style={{ padding: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px', borderRadius: '14px', background: '#fafbfe', border: '1px solid #edeef4' }}>
              <div style={{ flex: 1, marginRight: '16px' }}>
                ...
                <div style={{ fontSize: '0.94rem', fontWeight: 700, color: '#171433', marginTop: '3px' }}>{userName}</div>
              </div>
              <button
                onClick={() => {
                  const newName = window.prompt('Enter new name:', userName);
                  if (newName && newName.trim()) {
                    ...
                    localStorage.setItem('miracle_user', JSON.stringify(stored));
                    window.dispatchEvent(new CustomEvent('miracle_user_updated'));
                    api.updateProfile({ name: newName.trim() }).catch(() => {});
                  }
                }}
                style={{ padding: '7px 16px', borderRadius: '10px', border: '1px solid #edeef4', background: '#fff', fontSize: '0.78rem', fontWeight: 600, color: PUR, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', transition: 'background 0.15s' }}
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // ACCOUNT SETTINGS PAGE — Exact Admin Standard
  // ─────────────────────────────────────────────────────────────────────────
  const renderAccountSettingsPage = () => {
    const userName = profileName || storedUser?.name || 'Ananya Sharma';
    const userEmail = storedUser?.email || 'user@miracle.com';
    const [editField, setEditField] = React.useState<'name' | 'password' | null>(null);
    const [tempName, setTempName] = React.useState(userName);
    const [settingToast, setSettingToast] = React.useState<{ msg: string; ok: boolean } | null>(null);

    const showToast = (msg: string, ok: boolean) => {
      setSettingToast({ msg, ok });
      setTimeout(() => setSettingToast(null), 3000);
    };

    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {settingToast && (
          <div style={{ position: 'fixed', top: '20px', right: '24px', zIndex: 9999, padding: '12px 18px', borderRadius: '12px', background: settingToast.ok ? '#ecfdf5' : '#fef2f2', border: `1px solid ${settingToast.ok ? '#a7f3d0' : '#fecaca'}`, color: settingToast.ok ? '#065f46' : '#991b1b', fontWeight: 700, fontSize: '0.84rem', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>
            {settingToast.ok ? '✅' : '❌'} {settingToast.msg}
          </div>
        )}
        <Card style={{ padding: '24px' }}>
              ...
              <div style={{ flex: 1, marginRight: '16px' }}>
                ...
                {editField === 'name' ? (
                  <div style={{ display: 'flex', gap: '8px', marginTop: '6px' }}>
                    ...
                    <button
                      onClick={() => {
                        if (!tempName.trim()) { showToast('Name cannot be empty', false); return; }
                        ...
                        setEditField(null);
                        showToast('Name updated successfully!', true);
                      }}
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button
                        onClick={async () => {
                          ...
                          } catch (e: any) { showToast(e?.message || 'Failed to update password', false); }
                        }}
                        style={{ padding: '8px 14px', borderRadius: '8px', border: 'none', background: PUR, color: '#fff', fontWeight: 700, fontSize: '0.78rem', cursor: 'pointer' }}
                      >Update</button>
                      <button onClick={() => setEditField(null)} style={{ padding: '8px 12px', borderRadius: '8px', border: '1px solid #edeef4', background: '#fff', fontSize: '0.78rem', cursor: 'pointer' }}>Cancel</button>
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // MY SKIN PROFILE — With Illustrated Skin Type & Concern Images
  // ─────────────────────────────────────────────────────────────────────────
  const renderMySkinProfilePage = () => {
    const SKIN_TYPE_DATA: { type: string; emoji: string; desc: string; img: string }[] = [
      { type: 'Oily', emoji: '✨', desc: 'Excess sebum, enlarged pores, frequent breakouts', img: 'https://images.unsplash.com/photo-1616394584738-fc6e612e71b9?w=80&h=80&fit=crop&crop=face' },
      { type: 'Dry', emoji: '🌵', desc: 'Tight, flaky, dehydration & fine lines', img: 'https://images.unsplash.com/photo-1598440947619-2c35fc9aa908?w=80&h=80&fit=crop' },
      { type: 'Combination', emoji: '☯️', desc: 'Oily T-zone, dry cheeks, balanced care', img: 'https://images.unsplash.com/photo-1522337360788-8b13dee7a37e?w=80&h=80&fit=crop&crop=face' },
      { type: 'Sensitive', emoji: '🌸', desc: 'Reactive, prone to redness & irritation', img: 'https://images.unsplash.com/photo-1605462863863-10d9e47e15ee?w=80&h=80&fit=crop' },
      { type: 'Normal', emoji: '💎', desc: 'Balanced, minimal concerns, healthy glow', img: 'https://images.unsplash.com/photo-1614159102938-d9b05a3e6e37?w=80&h=80&fit=crop&crop=face' },
    ];

    const CONCERN_DATA: { name: string; emoji: string; img: string; desc: string }[] = [
      { name: 'Acne & Breakouts', emoji: '🎯', img: 'https://images.unsplash.com/photo-1560857792-215f9e3534ab?w=60&h=60&fit=crop', desc: 'Papules, pustules, comedonal acne' },
      { name: 'Hyperpigmentation', emoji: '🌑', img: 'https://images.unsplash.com/photo-1629909613654-28e377c37b09?w=60&h=60&fit=crop', desc: 'PIH, dark spots, uneven tone' },
      { name: 'Dryness & Barrier Loss', emoji: '💧', img: 'https://images.unsplash.com/photo-1598440947619-2c35fc9aa908?w=60&h=60&fit=crop', desc: 'Dehydration, flaking, TEWL' },
      { name: 'Redness & Rosacea', emoji: '🌹', img: 'https://images.unsplash.com/photo-1616394584738-fc6e612e71b9?w=60&h=60&fit=crop', desc: 'Flush, dilated capillaries, reactive skin' },
      { name: 'Fine Lines & Aging', emoji: '🕰️', img: 'https://images.unsplash.com/photo-1579218856297-d15d5e4e345e?w=60&h=60&fit=crop', desc: 'Photoaging, collagen loss, wrinkles' },
      { name: 'Enlarged Pores', emoji: '🔬', img: 'https://images.unsplash.com/photo-1616394584738-fc6e612e71b9?w=60&h=60&fit=crop', desc: 'Sebaceous filaments, pore dilation' },
      { name: 'Uneven Texture', emoji: '📐', img: 'https://images.unsplash.com/photo-1522337360788-8b13dee7a37e?w=60&h=60&fit=crop', desc: 'Rough surface, keratosis pilaris' },
    ];
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        {/* SKIN TYPE — Illustrated Image Tiles */}
        <Card style={{ padding: '22px' }}>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background: isSel ? '#f0effe' : '#fafafa',
                    cursor: 'pointer', textAlign: 'center',
                    transition: 'all 0.18s',
                    boxShadow: isSel ? `0 4px 16px ${PUR}25` : 'none',
                    outline: 'none',
                  }}
                  onMouseEnter={e => { if (!isSel) (e.currentTarget as HTMLElement).style.borderColor = PUR; }}
                ...
                >
                  <img
                    ...
                    style={{ width: '64px', height: '64px', borderRadius: '50%', objectFit: 'cover', display: 'block', margin: '0 auto 10px', border: `2px solid ${isSel ? PUR : '#e2e8f0'}` }}
                  />
                  <div style={{ fontSize: '0.92rem', fontWeight: 900, color: isSel ? PUR : '#0f172a', marginBottom: '4px' }}>
                    {isSel ? '✓ ' : ''}{st.emoji} {st.type}
                  </div>
                  <div style={{ fontSize: '0.7rem', color: '#64748b', lineHeight: 1.3 }}>{st.desc}</div>
                </button>
              );
            })}
          </div>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background: has ? PUR : '#fafafa',
                    cursor: 'pointer', textAlign: 'center',
                    transition: 'all 0.18s',
                    boxShadow: has ? `0 4px 14px ${PUR}30` : 'none',
                    outline: 'none',
                  }}
                  onMouseEnter={e => { if (!has) (e.currentTarget as HTMLElement).style.borderColor = PUR; }}
                  onMouseLeave={e => { if (!has) (e.currentTarget as HTMLElement).style.borderColor = '#e2e8f0'; }}
                >
                  <img
                    src={c.img}
                    alt={c.name}
                    onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }}
                    style={{ width: '52px', height: '52px', borderRadius: '50%', objectFit: 'cover', display: 'block', margin: '0 auto 8px', border: has ? '2px solid rgba(255,255,255,0.6)' : '2px solid #e2e8f0', filter: has ? 'brightness(1.1)' : 'none' }}
                  />
                  <div style={{ fontSize: '0.78rem', fontWeight: 800, color: has ? '#fff' : '#0f172a', marginBottom: '3px', lineHeight: 1.2 }}>{c.name}</div>
                  <div style={{ fontSize: '0.66rem', color: has ? 'rgba(255,255,255,0.8)' : '#64748b', lineHeight: 1.3 }}>{c.desc}</div>
                </button>
              );
            })}
          </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%', marginTop: '12px' }}>
                    <input
                      ...
                      style={{ fontSize: '0.78rem', color: '#475569', cursor: 'pointer' }}
                    />
                  </div>
                </div>
              )}
  ...
  const renderLifestylePage = () => {
    const [habitChecks, setHabitChecks] = React.useState<{ [key: string]: boolean }>({
    ...
    const [nutritionLog, setNutritionLog] = React.useState<{ [key: string]: boolean }>({
    ...
    const [airQuality, setAirQuality] = React.useState<number>(42);
    const [humidity, setHumidity] = React.useState<number>(58);
    ...
    const toggleNutr = (key: string) => {
      setNutritionLog(prev => ({ ...prev, [key]: !prev[key] }));
    };
          ...
          <Card style={{ padding: '22px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div>
                ...
                <input type="range" min="10" max="300" value={airQuality} onChange={e => setAirQuality(Number(e.target.value))} style={{ width: '100%', accentColor: '#059669' }} />
              </div>

              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', fontWeight: 700, marginBottom: '6px' }}>
                  <span>Ambient Humidity (%):</span>
                  <span style={{ color: BLU, fontWeight: 900 }}>{humidity}% (Ideal Barrier Range)</span>
                </div>
                <input type="range" min="10" max="100" value={humidity} onChange={e => setHumidity(Number(e.target.value))} style={{ width: '100%', accentColor: BLU }} />
              </div>
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '18px' }}>
          <Card style={{ padding: '22px', minHeight: '380px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
              <h3 style={{ margin: 0, fontSize: '1.05rem', fontWeight: 800, color: '#0f172a' }}>Score Progression Graph</h3>
              <span style={{ fontSize: '0.72rem', fontWeight: 700, color: '#059669', background: '#ecfdf5', padding: '3px 8px', borderRadius: '6px' }}>
                {scorePct !== null ? `Current: ${scorePct}/100` : 'No data yet'}
              </span>
            </div>
            <div style={{ height: '260px', paddingBottom: '8px' }}>
              <ChartFrame
                chart={{ el: <LineChart vals={chartVals} min={0} max={100} /> }}
                yLabels={['100', '75', '50', '25', '0']}
                xLabels={(() => {
                  const raw = analytics?.score_history?.length
                    ? analytics.score_history.map(h => h.date.slice(5))
                    : ['W1', 'W2', 'W3', 'Today'];
                  if (raw.length <= 6) return raw;
                  const step = Math.floor(raw.length / 5);
                  return [raw[0], raw[step], raw[step*2], raw[step*3], raw[step*4], raw[raw.length-1]];
                })()}
                h={260}
              />
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.74rem', color: '#64748b', marginTop: '8px', borderTop: '1px solid #f1f5f9', paddingTop: '8px' }}>
              <span>Longitudinal skin barrier trajectory</span>
              <span style={{ color: '#059669', fontWeight: 700 }}>▲ +14% vs Baseline</span>
            </div>
          </Card>
          ...
          {scanStep === 'ready' && (
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', justifyContent: 'center' }}>
              {!cameraActive ? (
                ...
                <button
                  onClick={stopCamera}
                  style={{ padding: '12px 20px', borderRadius: '12px', background: '#f1f5f9', color: '#475569', border: '1px solid #cbd5e1', fontSize: '0.88rem', fontWeight: 700, cursor: 'pointer' }}
                >
                  Close Camera
                </button>
              )}
